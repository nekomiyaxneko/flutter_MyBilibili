...
class GetUtilBilibili{
  ...
  static   getRecommend() async{
    try{
      ...
      HttpClient httpClient=new HttpClient();
      HttpClientRequest request=await httpClient.getUrl(
        Uri.parse("http://app.bilibili.com/x/feed/index?appkey=1d8b6e7d45233436&build=508000&login_event=0&mobi_app=android"));
      HttpClientResponse response=await request.close();
      var result=await response.transform(utf8.decoder).join();
      Map<String,dynamic> jsondata=json.decode(result);
      RecommendListModel recommendVideoListModel=RecommendListModel.fromJson(jsondata);
        ...
        for (var item in recommendVideoListModel.data){
          if(item.goto=="av"){
            ...
            videolist.add(VideoItem(
              ...
              view: item.play>10000?(item.play~/10000).toString()+"万":item.play.toString(),
              aid: item.param,
              danmu: item.danmaku>10000?(item.danmaku~/10000).toString()+"万":item.danmaku.toString(),
              id: getrandomhash(),
              tname: item.tname,
            ));
          }
  ...
  static durationtoString(int duration){
    if(duration>=3600){
      return (duration~/3600<10?"0${duration~/3600}":"${duration~/3600}")+":"+((duration~/60)%60<10?"0${(duration~/60)%60}":"${(duration~/60)%60}")+":"+(duration%60<10?"0${duration%60}":"${duration%60}");
    }
    else{
      return ((duration~/60)%60<10?"0${(duration~/60)%60}":"${(duration~/60)%60}")+":"+(duration%60<10?"0${duration%60}":"${duration%60}");
  ...
  static   getRankList(String tid,String page) async{
    try{
      HttpClient httpClient=new HttpClient();
      HttpClientRequest request=await httpClient.getUrl(
          Uri.parse("http://api.bilibili.cn/list?appkey=c1b107428d337928&pagesize=10&tid=${tid}&page=${page}"));
      HttpClientResponse response=await request.close();
      var result=await response.transform(utf8.decoder).join();
      Map<String,dynamic> jsondata=json.decode(result);
      RecommendListModel recommendVideoListModel=RecommendListModel.fromJson(jsondata);
  ...
  static  getLivePartition() async{//获取推荐直播列表
    try{
      List<LivePartition> livelist=List<LivePartition>();
      HttpClient httpClient=new HttpClient();
      HttpClientRequest request=await httpClient.getUrl(
          Uri.parse("http://api.live.bilibili.com/room/v1/AppIndex/getAllList?device=android&platform=android&scale=xhdpi"));
      HttpClientResponse response=await request.close();
      var result=await response.transform(utf8.decoder).join();
      Map<String, dynamic> jsondata=json.decode(result);
      //print("rpid "+jsondata["data"]["replies"][0]["rpid"].toString());
  ...
  static  getSearchByKeyword(String keyword,int pn) async{//获取搜索结果列表,pn为第几页,ps为页的大小
    try{
      List<VideoItem> searchresultlist=List<VideoItem>();
      HttpClient httpClient=new HttpClient();
      HttpClientRequest request=await httpClient.getUrl(
          Uri.parse("http://app.bilibili.com/x/v2/search?appkey=1d8b6e7d45233436&build=5370000&pn=${pn.toString()}&ps=15&keyword=${keyword}"));
      HttpClientResponse response=await request.close();
      var result=await response.transform(utf8.decoder).join();
      Map<String, dynamic> jsondata=json.decode(result);
      //print("rpid "+jsondata["data"]["replies"][0]["rpid"].toString());
  ...
  static  getChannelList() async{//获取频道列表//因为需要sign这个参数，暂时还不会计算，所以就直接用现成的数据
    try{
      List<ChannelItem> channellist=List<ChannelItem>();
      
      HttpClient httpClient=new HttpClient();
      HttpClientRequest request=await httpClient.getUrl(
          Uri.parse("http://app.bilibili.com/x/channel/square"+
  ...
  static  getMallList() async{//获取获取会员购列表
    try{
      List<GoodItem> goodllist=List<GoodItem>();
      HttpClient httpClient=new HttpClient();
      HttpClientRequest request=await httpClient.getUrl(
          Uri.parse("http://mall.bilibili.com/mall-c/home/index/v2?mVersion=17"));
      HttpClientResponse response=await request.close();
      var result=await response.transform(utf8.decoder).join();
      Map<String, dynamic> jsondata=json.decode(result);
      //print("rpid "+jsondata["data"]["replies"][0]["rpid"].toString());
      //print(jsondata);
      for(Map<String,dynamic> item in jsondata["data"]["vo"]["feeds"]["list"]){
        //print(item);
